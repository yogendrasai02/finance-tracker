...
| DM-33 | The two database roles and the `app` schema are created by a Postgres init script (`db/init/01-roles-and-schema.sh`) outside Flyway — run automatically on first container start locally, and explicitly as a CI step | `ft_migrator` must exist before Flyway's very first connection, so something outside Flyway has to create it. The same script is reused unchanged in both environments, so there is one source of truth for the roles rather than two that can drift |

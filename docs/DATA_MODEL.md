...
| Timestamps | `TIMESTAMPTZ`, defaulting to `now()`. Every table has `created_at`; a table whose rows change also has `updated_at` | Stores an absolute instant, not a local wall-clock reading (DM-25) |
...
| DM-31 | The base schema ships as four migrations, not one: `V1` tables/keys/constraints/indexes, `V2` triggers, `V3` Row-Level Security, `V4` seed data | DM-16 requires the base schema in one migration only so that a parent's `UNIQUE (user_id, id)` exists before a child's composite foreign key; all tables and foreign keys stay inside `V1`, so that rule still holds. Triggers and policies attach to tables that already exist and share no such ordering constraint, so four short files are reviewable one concern at a time |
| DM-32 | The application's tables live in a dedicated schema `app`, not `public`. The migration role (`ft_migrator`) owns the schema; the application role (`ft_app`) gets `USAGE` plus DML only, and `ALTER DEFAULT PRIVILEGES` grants every future table to `ft_app` automatically | Lets tenant-safe rights be granted on "everything in `app`" as one unit. No migration ever has to remember its own `GRANT` statement, which would otherwise be a place to forget one |
| DM-33 | The two database roles and the `app` schema are created by a Postgres init script (`db/init/01-roles-and-schema.sh`) outside Flyway — run automatically on first container start, by `docker compose` locally and by Testcontainers wherever the schema tests start their own container | `ft_migrator` must exist before Flyway's very first connection, so something outside Flyway has to create it. The same script is reused unchanged everywhere it runs, so there is one source of truth for the roles rather than several that can drift |
| DM-34 | Schema tests run against real Postgres (Testcontainers, `postgres:18-alpine`), never an in-memory database | H2 has no Row-Level Security, no partial unique indexes, no `plpgsql` triggers — exactly the features these tests exist to check. A suite that passed on H2 would prove nothing about the database the application actually uses |
| DM-35 | Schema tests open their own JDBC connections and name the role (`ft_app` or `ft_migrator`) explicitly, never through Spring's normal datasource | The tests must compare what each role is allowed to do. A test that happens to run as the schema owner passes even when RLS is completely broken, since the owner skips its own policies by design (DM-32) |

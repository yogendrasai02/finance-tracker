...
| D-33 | FR-8 is delivered as server-side sessions in an `HttpOnly` cookie via Spring Security. No JWT | SR-38 requires server-enforced logout and expiry, which a stateless token cannot give without a denylist that reintroduces server state |
| D-34 | Sessions are stored in Postgres, in a separate `auth` schema, via Spring Session JDBC. Idle timeout 30 minutes, absolute lifetime 12 hours, both server-enforced | Survives an application restart, unlike JVM memory. A session establishes the tenant, so it cannot itself be tenant-scoped without being circular — keeping it in a schema outside `app` keeps "every table in `app` is tenant-owned" true for every table that convention actually checks |
| D-35 | Login reads the user row through `app.find_login_identity`, a `SECURITY DEFINER` function, rather than weakening Row-Level Security or adding a `BYPASSRLS` role | The only narrow, reviewable exception to tenant isolation this project has; the alternatives are permanently wider |
| D-36 | The seeded user's password is bootstrapped from `FT_OWNER_EMAIL` and `FT_OWNER_PASSWORD` on startup, only when the account has no hash yet | SR-40 forbids a hash in a migration. A bootstrap is not a password reset, so a later `.env` change never overwrites a credential already in use |
| D-37 | Argon2id, through a `DelegatingPasswordEncoder` | OWASP's current first choice for password hashing. A future algorithm change re-hashes a user on their next login rather than needing a data migration |
| D-38 | The tenant id is applied by `TenantAwareJpaTransactionManager` at the start of every transaction, not by service code | Makes scoping structural rather than a per-query habit that one forgotten call can silently break. See [DATA_MODEL.md](DATA_MODEL.md) §2.3 |
| D-39 | CSRF tokens plus `SameSite=Strict`; one origin in development via a Vite dev-server proxy | Removes cross-port CORS-with-credentials from local development entirely, closing a class of "works locally, breaks in production" cookie bugs |
| D-40 | Login rate limiting is in-process (Bucket4j), keyed by client IP and the submitted email together; no account lockout | Enough for SR-39 without distributed infrastructure for one user. Lockout would be a denial-of-service switch anyone who knows the email address could flip |
| D-41 | The frontend stores no authentication state. `GET /api/v1/me` on load decides logged-in versus not | There is no token to store, so there is nothing to put in `localStorage` (SR-37). The cookie and the server are the only authority |
| D-42 | A dedicated `com.financetracker.auth` package owns the one login path that reads a user row with no tenant id | Keeps the privileged read physically separate from ordinary user code, so any change to it shows up in a diff against a directory that exists for exactly that reason |
...
**FR-8 — Authentication. Delivered in step 4.** Real login for the single seeded user, no registration, invites, or roles.
A session lives in an `HttpOnly`, `Secure`, `SameSite=Strict` cookie, backed by a row in Postgres (`auth.spring_session`) rather than JVM memory, with a 30-minute idle timeout and a 12-hour absolute lifetime, both enforced server-side.
Passwords are hashed with Argon2id; the seeded owner's credential is bootstrapped from the environment on first run, never written into a migration (SR-40).
Login is rate-limited and every failure reason returns the same response.
Every domain query runs inside a transaction that has already set `app.user_id`, so Row-Level Security scopes it without any service method remembering to.
See D-33 through D-42, and [SECURITY.md](SECURITY.md) §6.
Two constraints from D-31 are satisfied only in development so far, not in a real deployment: TLS on the browser→app hop is the hosting platform's job and is not yet configured anywhere, and CORS has no explicit origin allowlist yet because no production frontend origin exists (`plans/STATUS.md` tracks both).
